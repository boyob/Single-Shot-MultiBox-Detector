...
# loc       = [g_cx, g_cy, g_w, g_h], shape=[8732, 4], 范围：有正有负
# priors    = [d_cx, d_cy, d_w, d_h], shape=[8732, 4]，范围：[0,1]
# variances = [0.1, 0.2]
...
def test_decode():
    loc = torch.tensor([[0.6, -0.7, 1.5, -2.5]])
    priors = torch.tensor([[0.5, 0.4, 0.3, 0.2]])
    boxes = decode(loc, priors, variances=[0.1, 0.2])
    print(boxes)  # [0.3155, 0.3253, 0.7205, 0.4467]
